...
extern int dir;
extern bool special_key_pressed;
...
DrawingWindow::DrawingWindow(int argc, char** argv,int _width, int _height, int startX, int startY, string name)
{
	...
	glutSpecialUpFunc(specialFunctionUp); /* functie adaugata de mine pentru a trata eliberarea unei taste */
		...
		else 
		{
			if (objects2D[i]->points.size() >= 3) //daca are mai mult de 3 puncte
			{
				...
				if (!objects2D[i]->fill)
				{
					...
					for (j = 0; j < objects2D[i]->points.size();j++)
					{
...
void DrawingWindow::drawObjects(Visual2D *v)
{
	...
	//deseneaza obiecte 2D
	unsigned int i,j;
	for (i = 0; i < v->visual2D_objects2D.size(); i++)
	{
		//daca e linie
		...
		else 
		{
			if (v->visual2D_objects2D[i]->points.size() >= 3) //daca are mai mult de 3 puncte
			{
				...
				{
					glBegin(GL_LINE_LOOP); //se poate folosi si GL_LINES, dar trebuie date doua puncte o data
					for (j = 0; j < v->visual2D_objects2D[i]->points.size();j++)
					{
	...
	//deseneaza obiecte 3D
	unsigned int face,vertex;
	for (i=0; i < v->visual2D_objects3D.size(); i++)
	{
...
void DrawingWindow::displayFunction()
{
	...
	if (visuals2D.size() == 0)
		...
		for (unsigned int i = 0; i < visuals2D.size(); i++)
		{
...
void DrawingWindow::specialFunctionUp(int key, int x, int y)
{
	if (key == KEY_LEFT)
		pressed_left = false;
	if (key == KEY_RIGHT)
